...
class ConfigPopup:
    def __init__(self):
        ...
        self.should_put_to_monthly_playlist = BooleanVar(
            self.root, self.config.get("should_put_to_monthly_playlist", True)
        )
        self.should_heart_on_lastfm = BooleanVar(
            self.root, self.config.get("should_heart_on_lastfm", True)
        )
        self.currently_playing_trunclen = IntVar(
            self.root, self.config.get("currently_playing_trunclen", 45)
        )
        ...
        Label(self.root, text="Currently playing max length: ").grid(row=2, column=0)
        trunclen_entry_box = Entry(
            self.root, textvariable=self.currently_playing_trunclen, width=3
        )
        trunclen_entry_box.grid(row=2, column=1)
        trunclen_entry_box.bind(
            "<Return>", lambda event: self.handle_set_currently_playing_trunclen()
        )
    ...
    def handle_should_put_to_monthly_playlist_change(self):
        self.config.set(
            "should_put_to_monthly_playlist", self.should_put_to_monthly_playlist.get()
        )

    def handle_should_heart_on_lastfm(self):
        self.config.set("should_heart_on_lastfm", self.should_heart_on_lastfm.get())

    def handle_set_currently_playing_trunclen(self):
        self.config.set(
            "currently_playing_trunclen", int(self.currently_playing_trunclen.get())
        )
